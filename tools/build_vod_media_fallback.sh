...
rm -rf "$OUT" "$WORK" "$BASE"
mkdir -p "$OUT" "$WORK/base" "$WORK/runtime" "$WORK/smali-full" "$WORK/stub/com/secneo/apkwrapper" "$WORK/stub/com/google/firebase/provider" "$WORK/stub/androidx/core/content" "$WORK/primary-smali" "$WORK/dummy23"
...
.end method
EOF
cat > "$WORK/stub/androidx/core/content/FileProvider.smali" <<'EOF'
...
mkdir -p "$WORK/primary-smali/com/secneo/apkwrapper" "$WORK/primary-smali/com/google/firebase/provider" "$WORK/primary-smali/androidx/core/content"
...
cp "$WORK/stub/androidx/core/content/"*.smali "$WORK/primary-smali/androidx/core/content/"
...
echo 'FULL_68_DEX_PRIMARY_STUBS_FIREBASE_FILEPROVIDER_VOD_FALLBACK_BUILD_OK' | tee "$OUT/build-result.txt"
