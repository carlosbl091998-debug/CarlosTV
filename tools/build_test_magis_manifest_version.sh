...
# Decode resources but never disassemble/recompile classes.dex. With -s, apktool keeps the protected DEX opaque.
...
cp "$WORK/decoded/apktool.yml" "$OUT/apktool.before.yml"
# apktool stores package version metadata in apktool.yml rather than decoded AndroidManifest.xml.
python3 - "$WORK/decoded/apktool.yml" "$VERSION_CODE" <<'PY'
import re,sys
p,v=sys.argv[1:]
s=open(p,encoding='utf-8').read()
s2,n=re.subn(r'(?m)^(\s*versionCode:\s*)[^\r\n]+$',lambda m:m.group(1)+v,s,count=1)
if n != 1:
    raise SystemExit(f'versionCode replacements={n}')
open(p,'w',encoding='utf-8').write(s2)
PY
cp "$WORK/decoded/apktool.yml" "$OUT/apktool.after.yml"
...
# Best-effort grants so validation reaches application UI rather than stopping at Android permission UI.
