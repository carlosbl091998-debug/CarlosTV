...
# Recover the already-tested Xuper candidate from its Actions artifact.
...
test -n "$BASE_SRC"
cp "$BASE_SRC" "$BASE"
sha256sum "$BASE" | tee "$OUT/base-sha256.txt"
...
e=ET.SubElement(app,'provider')
e.set(ns+'name','com.xuper.vodfix.VodFixProvider')
e.set(ns+'authorities','com.msandroid.mobile.vodfix')
e.set(ns+'exported','false')
e.set(ns+'initOrder','999999')
t.write(p,encoding='utf-8',xml_declaration=True)
PY

# Gadget + self-contained script for the two ABIs shipped by the application.
for spec in 'arm64-v8a:arm64' 'armeabi-v7a:arm'; do
  abi=${spec%%:*}; frida_arch=${spec##*:}
  d="$WORK/decoded/lib/$abi"; mkdir -p "$d"
  curl -fL --retry 3 --retry-all-errors \
    "https://github.com/frida/frida/releases/download/${FRIDA_VERSION}/frida-gadget-${FRIDA_VERSION}-android-${frida_arch}.so.xz" \
    -o "$WORK/gadget-${abi}.xz"
...
(function () {
  ...
  function attempt() {
    ...
    Java.perform(function () {
      try {
        var Z = Java.use('cb.z1');
        var g = Z.g.overload('java.lang.Object','java.lang.String','boolean','java.lang.Class','java.lang.String','java.util.HashMap','java.lang.String','boolean');
        g.implementation = function (body, uri, secure, responseClass, method, headers, serviceName, flag) {
          ...
          if (u.indexOf('api/portalCore/v10/startPlayVOD') !== -1) {
            var patched = u.replace('api/portalCore/v10/startPlayVOD', 'api/portalCore/v9/startPlayVOD');
...
"$APKSIGNER" sign --ks "$KEYSTORE" --ks-pass pass:android --key-pass pass:android \
  --ks-key-alias androiddebugkey --out "$OUT/Xuper-VOD-v9-Gadget-Candidate.apk" "$WORK/aligned.apk"
"$APKSIGNER" verify --verbose --print-certs "$OUT/Xuper-VOD-v9-Gadget-Candidate.apk" > "$OUT/signing.txt"
sha256sum "$OUT/Xuper-VOD-v9-Gadget-Candidate.apk" | tee "$OUT/candidate-sha256.txt"
