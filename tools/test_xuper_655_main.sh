...
click_label() {
  local xml="$1" label="$2"
  python3 - "$xml" "$label" <<'PY' > /tmp/xuper_xy.txt || return 1
import re,sys,xml.etree.ElementTree as ET
root=ET.parse(sys.argv[1]).getroot(); needle=sys.argv[2].casefold()
for n in root.iter('node'):
    t=(n.attrib.get('text','')+' '+n.attrib.get('content-desc','')).strip().casefold()
...
echo '=== install original signed bootstrap ===' | tee "$OUT/actions.txt"
...
for cycle in 1 2 3 4 5 6; do
  ...
  click_label "$xml" 'Actualizar' || click_label "$xml" 'Update' || {
    echo 'UPDATE_BUTTON_NOT_FOUND' >&2
    exit 51
  }
  handle_installer "$before" "$cycle" || {
    echo 'UPDATE_DID_NOT_INSTALL' >&2
    exit 52
  }
done
...
[[ "$FINAL_CODE" == "$TARGET_CODE" || "$FINAL_NAME" == "$TARGET_NAME" ]] || {
  echo "WRONG_FINAL_VERSION expected=$TARGET_NAME/$TARGET_CODE got=$FINAL_NAME/$FINAL_CODE" >&2
  exit 53
}
