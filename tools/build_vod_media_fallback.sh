...
# The early live process exposed a 68-dex class path. runtime-023 is the SecNeo
# shell and is replaced with inert manifest compatibility stubs below. Preserve
# the recovered class-path order so framework/library dependencies resolve.
...
# Patch recovered app DEX #19 only.
...
# Manifest compatibility only: do not initialize SecNeo native shell.
...
.end method
EOF
java -jar "$SMALI_JAR" assemble "$WORK/stub" -o "$WORK/classes23.dex" > "$OUT/stub-assemble.txt" 2>&1
...
echo 'FULL_68_DEX_DIRECT_RUNTIME_VOD_FALLBACK_BUILD_OK' | tee "$OUT/build-result.txt"
