...
# Recovered runtime DEX set from the stable protected APK. runtime-018 contains
# SplashAty/framework classes; runtime-019 contains the app VOD logic.
...
test -s "$WORK/runtime18.dex"
test -s "$WORK/runtime19.dex"
checksum "$WORK/runtime18.dex" | tee "$OUT/runtime18-sha256.txt"
...
# Patch only the VOD quality gate in the complete recovered app DEX.
...
i = s.find(start)
if i < 0: raise SystemExit('PATCH_START_NOT_FOUND')
j = s.find(end, i)
if j < 0: raise SystemExit('PATCH_END_NOT_FOUND')
j += len(end)
...
if 'STATIC_VOD_FALLBACK' not in s2: raise SystemExit('PATCH_MARKER_MISSING')
open(out, 'w', encoding='utf-8').write(s2)
PY
cp "$OUT/g2-u-patched.smali" "$TARGET"
grep -F 'STATIC_VOD_FALLBACK' "$TARGET"
java -jar "$SMALI_JAR" assemble "$WORK/smali-full" -o "$WORK/classes2.dex" > "$OUT/smali-assemble.txt" 2>&1
test -s "$WORK/classes2.dex"

# The protected manifest names SecNeo wrapper classes. Replace the protected
# classes.dex entirely with the recovered runtime and provide inert manifest
# compatibility stubs so the native shell is never initialized.
...
.end method
EOF
java -jar "$SMALI_JAR" assemble "$WORK/stub" -o "$WORK/classes3.dex" > "$OUT/stub-assemble.txt" 2>&1
test -s "$WORK/classes3.dex"
...
cp "$WORK/runtime18.dex" "$WORK/classes.dex"
(cd "$WORK" && zip -q -u unsigned.apk classes.dex classes2.dex classes3.dex)
...
unzip -p "$CANDIDATE" classes.dex > "$OUT/classes1-final.dex"
unzip -p "$CANDIDATE" classes2.dex > "$OUT/classes2-final.dex"
unzip -p "$CANDIDATE" classes3.dex > "$OUT/classes3-final.dex"
